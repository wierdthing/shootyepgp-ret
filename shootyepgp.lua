sepgp = AceLibrary("AceAddon-2.0"):new("AceConsole-2.0", "AceHook-2.1", "AceDB-2.0", "AceDebug-2.0", "AceEvent-2.0",
    "AceModuleCore-2.0", "FuBarPlugin-2.0")
...
sepgp.timer.cd_text = ""
sepgp.timer:Hide()
sepgp.timer:SetScript("OnUpdate", function()
    sepgp.OnUpdate(this, arg1)
end)
sepgp.timer:SetScript("OnEvent", function()
end)
sepgp.alts = {}

local FLASK_BUFF_TEXTURES = {
    ["Interface\\Icons\\INV_Potion_62"] = true,  -- Flask of the Titans
    ["Interface\\Icons\\INV_Potion_41"] = true,  -- Supreme Power
    ["Interface\\Icons\\INV_Potion_97"] = true,  -- Distilled Wisdom
    ["Interface\\Icons\\INV_Potion_48"] = true,  -- Chromatic Resistance
    -- Add more if needed
}

local function UnitHasFlask(unit)
    for i = 1, 32 do
        local texture = UnitBuff(unit, i)
        if not texture then break end
        if FLASK_BUFF_TEXTURES[texture] then
            return true
        end
    end
    return false
...
function sepgp:buildMenu()
    if not (options) then
        ...
        options.args["ep_member"] = {
            type = "group",
            name = "+EPs to specific member",
            ...
            args = {
                award_name = {
                    type = "text",
                    name = "Player Name",
                    desc = "Enter the player name",
                    usage = "<PlayerName>",
                    get = function()
                        return sepgp._award_name or ""
                    end,
                    set = function(input)
                        sepgp._award_name = input or ""
                    end,
                    order = 1
                },
                award_amount = {
                    type = "text",
                    name = "EP Amount",
                    desc = "Enter the EP amount to award",
                    usage = "<EP>",
                    get = function()
                        return sepgp._award_amount or ""
                    end,
                    set = function(input)
                        local amount = tonumber(input)
                        local name = sepgp._award_name
                        if name and name ~= "" and amount and amount >= 0 and amount < sepgp.VARS.max then
                            sepgp:givename_ep(name, amount)
                            sepgp:refreshPRTablets()
                            sepgp._award_amount = ""
                            sepgp._award_name = ""
                        else
                            sepgp:defaultPrint("Invalid input. Please enter a valid name and EP amount.")
                        end
                    end,
                    validate = function(input)
                        local amount = tonumber(input)
                        local name = sepgp._award_name
                        return (name and name ~= "") and (amount and amount >= 0 and amount < sepgp.VARS.max)
                    end,
                    order = 2
                }
            }
        }
        options.args["ep_remove_member"] = {
            type = "group",
            name = "-EPs from specific member",
            desc = "Remove EPs from member.",
            order = 12,
            hidden = function()
                return not (admin())
            end,
            args = {
                remove_name = {
                    ...
                    usage = "<PlayerName>",
                    get = function()
                        return sepgp._remove_name or ""
                    end,
                    set = function(input)
                        sepgp._remove_name = input or ""
                    end,
                    order = 1
                },
                remove_amount = {
                    type = "text",
                    name = "EP Amount to Remove",
                    desc = "Enter the EP amount to remove",
                    usage = "<EP>",
                    get = function()
                        return sepgp._remove_amount or ""
                    end,
                    set = function(input)
                        local amount = tonumber(input)
                        local name = sepgp._remove_name
                        if name and name ~= "" and amount and amount >= 0 and amount < sepgp.VARS.max then
                            sepgp:givename_ep(name, -amount)
                            sepgp:refreshPRTablets()
                            sepgp._remove_amount = ""
                            sepgp._remove_name = ""
                        else
                            sepgp:defaultPrint("Invalid input. Please enter a valid name and EP amount to remove.")
                        end
                    end,
                    validate = function(input)
                        local amount = tonumber(input)
        ...
        options.args["ep_raid"] = {
            ...
            set = function(v)
                sepgp:award_raid_ep(tonumber(v))
            end,
            usage = "<EP>",
            hidden = function()
                return not (admin())
        ...
        options.args["ep_flask"] = {
            type = "text",
            name = "+EPs to Flask Users",
            desc = "Award EPs only to raid members with a Flask buff.",
            order = 61,
            get = "suggestedAwardEP",
        set = function(v)
            local amount = tonumber(v)
            if not amount or amount < 0 or amount >= sepgp.VARS.max then
                return
            end
            if GetNumRaidMembers() > 0 then
                local flaskUsers = {}
                for i = 1, GetNumRaidMembers() do
                    local name, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = GetRaidRosterInfo(i)
                    local unit = "raid" .. i
                    if name and UnitHasFlask(unit) then
                        table.insert(flaskUsers, name)
                        local main = sepgp_main
                        sepgp:givename_ep(main or name, amount)
                    end
                end
                if table.getn(flaskUsers) > 0 then
                    sepgp:Print(string.format("Awarded %d EP to flask-using raid members: %s", amount, table.concat(flaskUsers, ", ")))
                else
                    sepgp:Print("No raid members with Flask buff found.")
                end
            else
                sepgp:Print("You are not in raid.")
            end
        end,
            usage = "<EP>",
            hidden = function()
        ...
        options.args["ep_reserves"] = {
            ...
            get = "suggestedAwardEP",
            set = function(v)
                sepgp:award_reserve_ep(tonumber(v))
            end,
            usage = "<EP>",
            hidden = function()
                return not (admin())
        ...
        options.args["reserves"] = {
            ...
            order = 50,
            get = function()
                return (sepgp.reservesChannelID ~= nil) and (sepgp.reservesChannelID ~= 0)
            end,
            set = function(v)
                sepgp:reservesToggle(v)
            end,
            disabled = function()
                return (sepgp_main == nil)
            end
        }
        ...
        options.args["alts_percent"] = {
            ...
            order = 66,
            hidden = function()
                return (not sepgp_altspool) or (not IsGuildLeader())
            end,
            get = function()
                return sepgp_altpercent
            end,
            set = function(v)
                sepgp_altpercent = v
                if (IsGuildLeader()) then
        ...
        options.args["set_main"] = {
            ...
            set = function(v)
                sepgp_main = (sepgp:verifyGuildMember(v))
            end
        }
        ...
        options.args["reset"] = {
            ...
            func = function()
                StaticPopup_Show("SHOOTY_EPGP_CONFIRM_RESET")
            end
        }
    ...
    if (needInit) or (needRefresh) then
        ...
        self:debugPrint(string.format(L["Scanning %d members for EP/GP data. (%s)"], table.getn(members),
            (sepgp_raidonly and "Raid" or "Full")))
...
function sepgp:OnInitialize() -- ADDON_LOADED (1) unless LoD
    if sepgp_saychannel == nil then
        sepgp_saychannel = "GUILD"
    end
    if sepgp_decay == nil then
        sepgp_decay = sepgp.VARS.decay
    end
    if sepgp_minep == nil then
        sepgp_minep = sepgp.VARS.minep
    end
    if sepgp_progress == nil then
        sepgp_progress = "T1"
    end
    if sepgp_discount == nil then
        sepgp_discount = 0.25
    end
    if sepgp_altspool == nil then
        sepgp_altspool = false
    end
    if sepgp_altpercent == nil then
        sepgp_altpercent = 1.0
    end
    if sepgp_log == nil then
        sepgp_log = {}
    end
    if sepgp_looted == nil then
        sepgp_looted = {}
    end
    if sepgp_debug == nil then
        sepgp_debug = {}
    end
    if sepgp_pugEP == nil then
        sepgp_pugEP = {}
    end
    if sepgp_showRollWindow == nil then
...
function sepgp:OnEnable() -- PLAYER_LOGIN (2)
    ...
    sepgp.extratip = (sepgp.extratip) or
                         CreateFrame("GameTooltip", "shootyepgp_tooltip", UIParent, "GameTooltipTemplate")
...
function sepgp:AceEvent_FullyInitialized() -- SYNTHETIC EVENT, later than PLAYER_LOGIN, PLAYER_ENTERING_WORLD (3)
    ...
    if not IsAddOnLoaded("pfUI-addonskins") then
        if (pfUI) and pfUI.api and pfUI.api.CreateBackdrop and pfUI_config and pfUI_config.tooltip and
            pfUI_config.tooltip.alpha then
...
function sepgp:TipHook()
    ...
    self:HookScript(GameTooltip, "OnHide", function()
        if sepgp.extratip:IsVisible() then
            sepgp.extratip:Hide()
        end
...
function sepgp:delayedInit()
    ...
    if sepgp_reservechannel == nil then
        sepgp_reservechannel = sepgp.VARS.reservechan
    end
    ...
    self:RegisterChatCommand({"/updatepugep"}, function()
        sepgp:updateAllPugEP()
    end)
    ...
    if (fromAddon) then
        ...
        if (sepgp_altspool) then
            ...
            if oldmain ~= nil then
                if main == nil or main ~= oldmain then
                    ...
                    self:defaultPrint(string.format(L["|cffff0000Manually modified %s\'s note. Previous main was %s|r"],
                        name, oldmain))
                end
            end
        end
        if oldepgp ~= nil then
            if epgp == nil or epgp ~= oldepgp then
                ...
                self:defaultPrint(string.format(L["|cffff0000Manually modified %s\'s note. EPGP was %s|r"], name,
                    oldepgp))
            end
        end
    ...
    if LootSlotIsItem(slot) and quality >= 3 then
        ...
        if (itemLink) then
            ...
            elseif button == "MiddleButton" then
                self:widestAudience(string.format(L["Whisper %s a + or - for %s (mainspec or offspec)"],
                    sepgp._playerName, itemLink))
            end
        end
    end
end
    ...
    if (itemLink) then
        ...
        if (link_found) then
            ...
            if (bind == self.VARS.boe) then
                if button == "LeftButton" then
                    self:widestAudience(
                        string.format(L["Whisper %s a + for %s (mainspec)"], sepgp._playerName, itemLink))
                    return
                elseif button == "RightButton" then
                    self:widestAudience(string.format(L["Whisper %s a - for %s (offspec)"], sepgp._playerName, itemLink))
                    return
...
function sepgp:addonComms(prefix, message, channel, sender)
    if not prefix == self.VARS.prefix then
        return
    end -- we don't care for messages from other addons
    if sender == self._playerName then
        return
    end -- we don't care for messages from ourselves
    local name_g, class, rank = self:verifyGuildMember(sender, true)
    if not (name_g) then
        return
    end -- only accept messages from guild members
    ...
    if (who) and (what) and (amount) then
        ...
        elseif who == "SETTINGS" then
            for progress, discount, decay, minep, alts, altspct in string.gfind(what,
                "([^:]+):([^:]+):([^:]+):([^:]+):([^:]+):([^:]+)") do
                ...
                if (settings_notice) and settings_notice ~= "" then
                    ...
                    self._options.args["progress_tier_header"].name =
                        string.format(L["Progress Setting: %s"], sepgp_progress)
                    self._options.args["set_discount_header"].name =
                        string.format(L["Offspec Price: %s%%"], sepgp_discount * 100)
...
function sepgp:shareSettings(force)
    ...
    if self._lastSettingsShare == nil or (now - self._lastSettingsShare > 30) or (force) then
        ...
        local addonMsg = string.format("SETTINGS;%s:%s:%s:%s:%s:%s;1", sepgp_progress, sepgp_discount, sepgp_decay,
            sepgp_minep, tostring(sepgp_altspool), sepgp_altpercent)
        self:addonMessage(addonMsg, "GUILD")
    end
end
...
function sepgp:get_gp_v2(getname, officernote) -- gets gp by name or officernote
    if (officernote) then
        if tonumber(officernote) == nil then
...
function sepgp:award_raid_ep(ep) -- awards ep to raid members in zone
    if GetNumRaidMembers() > 0 then
        ...
        UIErrorsFrame:AddMessage(L["You aren't in a raid dummy"], 1, 0, 0)
    end
end
...
function sepgp:decay_epgp_v2() -- decays entire roster's ep and gp
    ...
    self:simpleSay(msg)
    if not (sepgp_saychannel == "OFFICER") then
        self:adminSay(msg)
    end
    self:addToLog(msg)
end
...
function sepgp:my_epgp_announce(use_main)
    ...
    if pr_decay < 0 then
        msg = string.format(L["Close to EPGP Cap. Next Decay will change your |cffff7f00PR|r by |cffff0000%.4g|r."],
            pr_decay)
        self:defaultPrint(msg)
    end
end
...
function sepgp:captureReserveChatter(text, sender, _, _, _, _, _, _, channel)
    if not (channel) or not (channel == sepgp_reservechannel) then
        return
    end
                ...
                if reserve_alt then
                    if not reserves_blacklist[reserve_alt] then
                        ...
                        self:defaultPrint(string.format(
                            L["|cffff0000%s|r trying to add %s to Reserves, but has already added a member. Discarding!"],
                            reserve_alt, reserve))
                    end
                else
                    if not reserves_blacklist[reserve] then
                        ...
                        self:defaultPrint(string.format(
                            L["|cffff0000%s|r has already been added to Reserves. Discarding!"], reserve))
                    end
                end
            end
        end
...
---------
local lootCall = {}
lootCall.whisp = {"^(w)[%s%p%c]+.+", ".+[%s%p%c]+(w)$", ".+[%s%p%c]+(w)[%s%p%c]+.*", ".*[%s%p%c]+(w)[%s%p%c]+.+",
                  "^(whisper)[%s%p%c]+.+", ".+[%s%p%c]+(whisper)$", ".+[%s%p%c]+(whisper)[%s%p%c]+.*",
                  ".*[%s%p%c]+(whisper)[%s%p%c]+.+", ".+[%s%p%c]+(bid)[%s%p%c]*.*", ".*[%s%p%c]*(bid)[%s%p%c]+.+"}
lootCall.ms = {".+(%+).*", ".*(%+).+", "^(ms)[%s%p%c]+.+", ".+[%s%p%c]+(ms)$", ".+[%s%p%c]+(ms)[%s%p%c]+.*",
               ".*[%s%p%c]+(ms)[%s%p%c]+.+", ".+(mainspec).*", ".*(mainspec).+"}
lootCall.os = {".+(%-).*", ".*(%-).+", "^(os)[%s%p%c]+.+", ".+[%s%p%c]+(os)$", ".+[%s%p%c]+(os)[%s%p%c]+.*",
               ".*[%s%p%c]+(os)[%s%p%c]+.+", ".+(offspec).*", ".*(offspec).+"}
    ...
    if (playerState ~= nil and targetState ~= nil) and playerState == 1 and targetState == 1 then
        ...
        if (itemLink) then
            ...
            if (link_found) then
                ...
                if (not bind) or (bind ~= self.VARS.boe) then
                    return
                end
                if UnitExists("target") and UnitIsPlayer("target") and UnitCanCooperate("player", "target") and
                    (not UnitIsUnit("player", "target")) then
...
function sepgp:processLootDupe(player, itemName, source)
    ...
    if ((self._lastPlayerItem) and self._lastPlayerItem == player_item) and
        ((self._lastPlayerItemTime) and (now - self._lastPlayerItemTime) < 3) and
        ((self._lastPlayerItemSource) and self._lastPlayerItemSource ~= source) then
...
function sepgp:verifyGuildMember(name, silent)
    for i = 1, GetNumGuildMembers(1) do
        local g_name, g_rank, g_rankIndex, g_level, g_class, g_zone, g_note, g_officernote, g_online =
...
function sepgp:parseVersion(version, otherVersion)
    ...
    if (otherVersion) then
        if not sepgp._otherversion then
            sepgp._otherversion = {}
        end
...
StaticPopupDialogs["SHOOTY_EPGP_CLEAR_LOOT"] = {
    ...
    OnCancel = function(_, reason)
        if reason == "clicked" then
            ...
            sepgp:defaultPrint(
                L["Loot info can be cleared at any time from the Tablet context menu or '/shooty clearloot' command"])
        end
    end,
    timeout = 0,
    whileDead = 1,
...
local sepgp_auto_gp_menu = { -- {text = "Choose an Action", isTitle = true},
{
...
function sepgp:EasyMenu(menuList, menuFrame, anchor, x, y, displayMode, level)
    ...
    UIDropDownMenu_Initialize(menuFrame, function()
        sepgp:EasyMenu_Initialize(level, menuList)
    end, displayMode, level)
...
-- GLOBALS: sepgp_saychannel,sepgp_groupbyclass,sepgp_groupbyarmor,sepgp_groupbyrole,sepgp_raidonly,sepgp_decay,sepgp_minep,sepgp_reservechannel,sepgp_main,sepgp_progress,sepgp_discount,sepgp_altspool,sepgp_altpercent,sepgp_log,sepgp_dbver,sepgp_looted,sepgp_debug,sepgp_fubar,sepgp_showRollWindow
